...
def get_humanoid_retarget_indices() -> jnp.ndarray:
    ...
    soma_joint_retarget_indices = []

    # NOTE: the order matters here.
    for human_name, soma_name in [
        ("pelvis", "Hips"),
        ("left_hip", "LeftLeg"),
        ("right_hip", "RightLeg"),
        ("left_knee", "LeftShin"),
        ...
        # SOMA's *Shoulder body is the clavicle segment; the child *Arm body
        # origin is the glenohumeral joint corresponding to the SMPL shoulder.
        ...
        soma_joint_retarget_indices.append(SOMA_LINK_NAMES.index(soma_name))

    soma_joint_retarget_indices = jnp.array(soma_joint_retarget_indices)

    return human_retarget_names, soma_joint_retarget_indices


human_retarget_names, soma_joint_retarget_indices = None, None
...
def load_motion_data(
    ...
    # SOMA23 is human-scale, so preserve the SMPL keypoints in metric units.
    if source_type == "smpl":
        pass
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=str(
            SCRIPT_DIR / "../protomotions/data/assets/urdf/for_retargeting/soma23.urdf"
        ),
    ...
    parser.add_argument(
        "--mesh-dir",
        type=str,
        default=str(SCRIPT_DIR / "../protomotions/data/assets/urdf/for_retargeting"),
    ...
    if SOMA_LINK_NAMES is None:
        SOMA_LINK_NAMES = list(robot.links.names)

    global human_retarget_names, soma_joint_retarget_indices
    human_retarget_names, soma_joint_retarget_indices = get_humanoid_retarget_indices()

    current_motion_index = 0

    # Create connectivity matrix for the G1 joints
    n_retarget = len(soma_joint_retarget_indices)
    soma_retarget_mask = jnp.zeros((n_retarget, n_retarget))
    for link_a, link_b, weight in direct_pairs:
        ...
        soma_retarget_mask = soma_retarget_mask.at[retarget_idx_a, retarget_idx_b].set(
            weight
        )
        soma_retarget_mask = soma_retarget_mask.at[retarget_idx_b, retarget_idx_a].set(
            weight
        )
    ...
    if args.visualize:
        ...
        def generate_trajectory():
            ...
            Ts_world_root, joints = solve_retargeting(
                ...
                soma_joint_retarget_indices=soma_joint_retarget_indices,
                soma_retarget_mask=soma_retarget_mask,
...
def foot_contact_cost(
    ...
    soma_joint_retarget_indices: jnp.ndarray,
    ...
    left_ankle_robot_idx = soma_joint_retarget_indices[left_ankle_idx]
    right_ankle_robot_idx = soma_joint_retarget_indices[right_ankle_idx]
    left_foot_robot_idx = soma_joint_retarget_indices[left_foot_idx]
    right_foot_robot_idx = soma_joint_retarget_indices[right_foot_idx]
...
def solve_retargeting(
    ...
    soma_joint_retarget_indices: jnp.ndarray,
    soma_retarget_mask: jnp.ndarray,
    ...
    joints_to_move_less = jnp.array(
        [
            ...
            for name in ["Neck1_y", "Neck1_z", "RightHand_z", "LeftHand_z"]
        ]
    )
    ...
    # Variables.
    class SimplifiedJointsScaleVarSOMA(
    ...
    var_joints_scale = SimplifiedJointsScaleVarSOMA(jnp.zeros(timesteps))
    ...
    def retargeting_cost(
        ...
        var_Ts_world_root: jaxls.SE3Var,
        var_robot_cfg: jaxls.Var[jnp.ndarray],
        var_joints_scale: SimplifiedJointsScaleVarSOMA,
        ...
        robot_pos = T_world_link.translation()[jnp.array(soma_joint_retarget_indices)]
    ...
    def pc_alignment_cost(
        ...
        T_root_link = jaxlie.SE3(robot.forward_kinematics(cfg=robot_cfg))
        T_world_link = T_world_root @ T_root_link
        link_pos = T_world_link.translation()[soma_joint_retarget_indices]

        # hand_aux_link 1
        left_wrist = human_retarget_names.index("left_wrist")
        left_wrist_idx = soma_joint_retarget_indices[left_wrist]
        ...
        right_wrist_idx = soma_joint_retarget_indices[right_wrist]
        ...
        right_hand_aux_pos = (
            ...
            + link_rot_mat_right_wrist @ jnp.array([0.2, 0.0, 0.0])
        )

        # torso aux link
        torso_idx = SOMA_LINK_NAMES.index("Hips")
        ...
        # Unlike G1, SOMA23 is human-proportioned and has full shoulder/elbow
        # articulation. Do not downweight elbows: doing so lets the optimizer
        # hit wrist targets through anatomically implausible whole-arm twists.
