...
def env_config(robot_cfg: RobotConfig, args) -> EnvConfig:
    ...
    observations = {
        ...
        "ego_visible_scene_pointcloud": ego_visible_scene_pointcloud_obs_factory(
            ...
            num_samples=args.scene_pointcloud_candidates,
            horizontal_fov_deg=90.0,
            vertical_fov_deg=60.0,
            near_m=0.05,
            far_m=6.0,
            # Causal scene memory: current visible geometry plus every static
            # surface observed since this clip episode began at frame zero.
    ...
    return EnvConfig(
        ...
        # A sequence-overfit episode always starts at clip frame zero. Random
        # phase resets would destroy the meaning of the complete trajectory.
        motion_manager=MimicMotionManagerConfig(
            init_start_prob=1.0,
            resample_on_reset=args.ego_camera_file is not None,
        ),
    )
...
def agent_config(robot_config, env_config, args):
    ...
    return DiscretePriorPEFTSFTAgentConfig(
        ...
        evaluator=EvaluatorConfig(eval_metrics_every=args.eval_metrics_every),
